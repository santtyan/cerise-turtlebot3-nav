...
WORLD=/c/Projetos/world_with_camera.model
...
gzserver --verbose -s libgazebo_ros_init.so -s libgazebo_ros_factory.so "" > /tmp/gz_camera.log 2>&1 &
GZPID=
...
ros2 run gazebo_ros spawn_entity.py -entity robot1 -file "" -robot_namespace robot1 -x 0.0 -y 0.5 -z 0.01 2>&1 | grep status
ros2 run gazebo_ros spawn_entity.py -entity robot2 -file "" -robot_namespace robot2 -x 0.0 -y -0.5 -z 0.01 2>&1 | grep status
