...
Gazebo and hardware), with $34\%$ RMSE reduction for their decentralized
scheme and $56\%$ for a dual-landmark variant — comparable in order of
magnitude to our $+23.7\%$. Ahmed and
...
We do not evaluate against a public multi-robot benchmark such as UTIAS
MRCLAM~\cite{leung2011utias} or S3E~\cite{feng2022s3e}: both assume a
sensor payload carried by each robot (range-bearing or onboard
LiDAR/stereo/IMU), whereas our setup observes robots from a single
external, static overhead camera — closer to a fixed smart-environment
sensor than to cooperative SLAM. To our knowledge no public dataset
combines a fixed external camera, YOLO-based detection, and multiple
ground robots as fusion targets, which motivates the recorded dataset
released with this work (Section~\ref{sec:experimental-setup}).
...
synthetic checkerboard at multiple poses. A first calibration attempt
with all four intrinsics free reached low reprojection error (0.162px)
but an ill-conditioned focal length ($f_x$ 33\% above the FOV-derived
theoretical value) — the degenerate case Zhang's method is known to
suffer from when the checkerboard plane stays fronto-parallel to the
sensor across all poses, which our fixed downward-looking camera and
ground-plane board cannot avoid. We corrected this by fixing $f_x,f_y,
c_x,c_y$ at their geometric nominal values and calibrating only lens
distortion (\texttt{CALIB\_FIX\_FOCAL\_LENGTH},
\texttt{CALIB\_FIX\_PRINCIPAL\_POINT}), the standard remedy when nominal
intrinsics are already known; the corrected calibration reproduces the
FOV-derived heuristic projection already used in production to within
0.001m position error, confirming both are geometrically equivalent.
...
by mean error and $13.4\%$ worse by RMSE (Wilcoxon signed-rank,
$p<0.001$) — the opposite sign from the correction-instant result, with
RMSE confirming the same reversal rather than being an artifact of the
mean-error metric.
...
The error reported above follows the Absolute Trajectory Error (ATE)
convention of Sturm et al.~\cite{sturm2012tum} (adopted as the de facto
protocol by the \texttt{evo} evaluation toolkit): per-sample position
error against ground truth, without a rigid alignment step, which is
appropriate here because the estimate, odometry, and ground truth already
share a common world frame via the fixed overhead camera (unlike SLAM,
where the estimated trajectory has a free gauge that must be aligned
before comparison). ATE alone cannot distinguish whether the degradation
comes from error accumulating over time or from each individual step
already drifting more than odometry. We therefore also report Relative
Pose Error (RPE)~\cite{sturm2012tum} — the error of the position delta
between consecutive readings, which isolates local, step-to-step drift
from the accumulated global error ATE captures. Aggregated across all
three scenarios under aggressive drift, RPE is $4.2\%$ worse than raw
odometry by mean error and $4.8\%$ worse by RMSE — the same sign as ATE,
confirming local drift also degrades, but a substantially smaller
magnitude than the $12.7\%$/$13.4\%$ ATE degradation. This indicates the
reversal is driven mostly by error accumulating over time under sparse
correction, not by each individual step drifting sharply worse than
odometry alone.
