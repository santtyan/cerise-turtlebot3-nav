...
% Espaçamento de \paragraph: o padrão do llncs.cls deixa uma folga
% vertical perceptível antes de cada título run-in; comprimido aqui
% para bater com o espaçamento compacto de figura/tabela acima (é o
% comando \paragraph padrão, não um rótulo em negrito manual, então
% seu ritmo vertical é um comprimento a ajustar, não uma escolha
% estrutural a questionar).
\makeatletter
...
\begin{abstract}
Sistemas multi-robô que combinam odometria e detecção visual externa
costumam usar um fallback discreto: detecção quando disponível,
odometria caso contrário, descartando incerteza e deixando o
comportamento indefinido em lacunas de detecção. Substituímos isso por
um EKF por robô, fundindo odometria e detecção YOLO de uma câmera fixa
suspensa, validado em duas etapas: consistência estatística sintética
(NEES/NIS), depois dados reais de um gêmeo digital ROS2/Gazebo com
três TurtleBot3.

O resultado depende de como o erro é medido. No instante de correção,
o filtro reduz o erro de posição em $+23,7\%$ sob deriva agressiva, o
ganho que a literatura tipicamente reporta. Mas, medido continuamente,
a cada leitura de odometria, o mesmo filtro é $12,7\%$ \emph{pior} que
a odometria bruta, sob a cobertura visual esparsa
(0--28\%) que nossa câmera exibe na prática. Essa reversão é esperada
ao operar abaixo de uma taxa crítica de observação, não uma falha de
projeto, conectando-se a um teto de covariância que estabiliza o
gating de Mahalanobis. Reportar as duas
métricas, não só o ganho pontual comumente citado, é necessário para
caracterizar honestamente a fusão sob observação esparsa; filtros
alternativos (UKF, partículas, IMM) também não resolvem, pois a
limitação vem da taxa de observação, não do filtro.
...
\paragraph{Predição e teto de covariância.}
O ruído de processo é
...
\paragraph{Associação de dados e correção.}
...
\paragraph{Calibração de câmera.}
Os intrínsecos vêm de
\texttt{cv2.calibrateCamera} (método de Zhang). Uma
primeira tentativa com os quatro intrínsecos livres alcançou baixo
erro de reprojeção (0,162\,px), mas uma distância focal mal-condicionada
...
heurística já usada em produção com erro de até 0,001\,m, confirmando
que ambas são geometricamente equivalentes.

\paragraph{Validação.}
Seguindo os testes de consistência padrão para
...
\paragraph{Configuração experimental.}
...
\paragraph{Erro no instante de correção.}
...
\paragraph{Erro contínuo: o achado central.}
