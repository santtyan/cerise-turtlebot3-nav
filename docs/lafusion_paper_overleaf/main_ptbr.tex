...
\title{Caracterização da Fusão por Filtro de Kalman Estendido entre Detecção Visual e Odometria sob Observação Esparsa: um Estudo de Caso em Gêmeo Digital Multi-Robô}
...
\begin{abstract}
Sistemas multi-robô que combinam odometria com detecção visual externa
costumam recorrer a um fallback discreto: confiar na detecção quando
disponível, na odometria caso contrário. Isso descarta informação de
incerteza e não oferece comportamento principiado durante lacunas de
detecção. Substituímos isso por um Filtro de Kalman Estendido (EKF)
por robô, fundindo odometria e detecção visual baseada em YOLO a
partir de uma câmera fixa suspensa, validado em duas etapas (NEES/NIS
sintético, depois dados reais gravados) em um gêmeo digital
ROS2/Gazebo com três robôs TurtleBot3. Medido nos instantes de
correção, o filtro reduz o erro de posição em $+23,7\%$ em relação à
odometria isolada sob deriva agressiva. Medido continuamente, a cada
leitura de odometria, o mesmo filtro é $12,7\%$ \emph{pior} que a
odometria bruta sob a cobertura visual esparsa (0--28\%) que nossa
câmera exibe na prática. Mostramos que essa reversão é a consequência
...
Mahalanobis. Reportar ambas as métricas, em vez de apenas o ganho no
instante de correção comumente reportado, é necessário para
caracterizar honestamente a fusão por EKF sob observação esparsa;
também discutimos por que filtros alternativos (UKF, filtro de
partículas, IMM) não resolvem a limitação subjacente.
...
instalar uma carga sensorial dedicada em cada robô é custoso ou
...
combinar duas estimativas de posição ruidosas, chegando em taxas
diferentes, nem sempre disponíveis no mesmo instante. Uma solução
comum e pragmática, usada no gêmeo digital estudado aqui antes de
nossa contribuição, é um \emph{fallback discreto}: confiar na detecção
quando associada com confiança, na odometria caso contrário. Isso
descarta informação: nenhuma correção parcial a partir de detecções de
baixa confiança, nenhuma propagação de incerteza, nenhum comportamento
principiado durante trechos sem detecção.

A Filtragem de Kalman Estendida (EKF) é a resposta clássica,
oferecendo predição/correção probabilística completa com propagação
explícita de incerteza. Suas propriedades de convergência, no entanto,
são classicamente estabelecidas sob observação razoavelmente densa e
regular~\cite{dissanayake2001slam}, uma suposição que uma única câmera
fixa suspensa, sujeita a campo de visão finito e auto-oclusão durante
manobras, não satisfaz em geral. Se isso importa na prática é a
pergunta que este trabalho investiga.
...
no mesmo gêmeo digital. Nossa contribuição é dupla: (1) validado em
duas etapas (NEES/NIS sintético, depois dados reais gravados), o
filtro entrega um ganho de fusão real ($+23,7\%$ de redução do erro de
posição nos instantes de correção, sob deriva agressiva); (2) o mesmo
filtro medido continuamente, não apenas nos instantes de correção, é
$12,7\%$ \emph{pior} que a odometria bruta sob a cobertura visual
esparsa (0--28\%) que nossa câmera exibe na prática. Explicamos essa
reversão mecanicamente, conectando-a à teoria já estabelecida sobre
filtragem de Kalman sob observações
...
crítica; nosso Cenário~3 (0\% de cobertura) situa-se no caso limite
$p\to0$, de modo que a saturação observada é teoricamente esperada,
não um sintoma de má configuração. Censi~\cite{censi2009geometric}
...
Khosravi et al.~\cite{khosravi2026cooperative} reportam localização
cooperativa descentralizada para dois robôs (correções mútuas via
LiDAR), com redução de RMSE de $34\%$/$56\%$, comparável em ordem de
grandeza ao nosso $+23,7\%$. Ahmed e
Fr\'emont~\cite{ahmed2026maritime} fundem um detector YOLOv8-RGBDis
customizado com um EKF por alvo para uma equipe multi-robô marítima
($\mathrm{mAP@0.5}=0,995$, numericamente próximo do nosso). Nenhum dos
dois reporta um regime de erro contínuo em que a fusão perde para a
linha de base sem fusão: o achado diferenciador deste trabalho.
Dobrynin e Garcia~\cite{dobrynin2025turtlebot} compartilham nossa
plataforma exata (três TurtleBot3, ROS2), mas resolvem localização
relativa sem câmera, portanto não encontram cobertura visual esparsa
(revisitado como trabalho complementar na
Seção~\ref{sec:future-work}). Nossa contribuição é o relato conjunto
do ganho no instante de correção e da degradação do erro contínuo,
fundamentado em \cite{sinopoli2004intermittent,censi2009geometric}
para explicar por que esse trade-off é esperado, e não um bug de
implementação. Também não avaliamos contra um benchmark multi-robô
público como UTIAS MRCLAM~\cite{leung2011utias} ou
S3E~\cite{feng2022s3e}: ambos assumem uma carga sensorial transportada
por cada robô, enquanto nossa configuração observa os robôs a partir
de uma única câmera externa e estática, mais próxima de um sensor
fixo de ambiente inteligente do que de SLAM cooperativo. Até onde
sabemos, nenhum conjunto de dados público combina esses três
elementos, o que motiva o conjunto de dados gravado e disponibilizado
com este trabalho (Seção~\ref{sec:experimental-setup}). Esta cobertura
reflete busca direcionada, não uma revisão sistemática da literatura
com protocolo registrado; uma revisão formal continua em aberto.
...
\texttt{cv2.calibrateCamera} (Zhang~\cite{zhang2000calibration}). Uma
...
filtros de Kalman~\cite{barshalom2001estimation}, checamos primeiro
NEES e NIS em simulação antes de confiar no filtro em dados reais: 30
...
\subsection{Erro no Instante de Correção}
...
leituras 2D não utilizadas; correções mútuas via LiDAR (no espírito de
Khosravi et al.~\cite{khosravi2026cooperative}) poderiam mirar
diretamente o regime de cobertura zero; (c) \textbf{incerteza de
projeção formal}: propagar o Jacobiano completo de projeção para $R$;
(d) \textbf{localização relativa distribuída}: a filtragem distribuída
sem câmera de Dobrynin e Garcia~\cite{dobrynin2025turtlebot} na mesma
plataforma poderia fornecer um segundo canal de correção durante
lacunas de cobertura visual.
